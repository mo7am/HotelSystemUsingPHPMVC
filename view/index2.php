...
<html lang="en">
  ...
  $result = $object->query($query);
        ...
        while ($assoc = $object->query_associ($result))
        {
           ...
           echo '<span class="photo"><img class="img-responsive" style="width: 50px ; height: 80px ; float: left ; border-radius: 50% ; " alt="avatar" src="assets/img/ui-zac.jpg"></span>';
           echo '<span class="subject">';
           echo '<br>';
           echo  '<span style="font-size:20px" class="from">'.$name.'</span>';
           echo '<br>';
           echo '<br>';
           echo '<span style="float:right" class="time">'.$mes->time.'</span>';
           echo '<br>';
           echo '<br>';
          ...
          <section class="wrapper">
              ...
              <div class="row">
                  <div class="col-lg-9 main-chart">
                  	...
                  	<?php
                            $dat = new database();  
                            $query="SELECT roomnum ,roomtype_id FROM room WHERE state_id = '1' AND state_clean_id = '5'";

                            $result = $dat->query($query);

                            while ($row = $dat->query_associ($result)) {
                                $storeArray =  $row['roomnum'];
                                $storeArray2 = $row['roomtype_id'];

                                $query2="SELECT type FROM roomtype WHERE ID = '$storeArray2'";
                                $related = $dat->query($query2);
                                while ($row = $dat->query_associ($related)) {
                                $storeArray3 =  $row['type'];
                                
                                echo  '<p style="background-color:black ; color:white ;width:300px ;font-size:25px ;height:40px" >'.$storeArray.'</p>'.'<div style="color:black ;font-size:25px">|---------------------------------------------></div>'.
                                        '<p style="background-color:black ; color:white ;width:300px ;font-size:25px ;margin-left:400px;height:40px ; margin-top:-30px" >'.$storeArray3.'</p>';
